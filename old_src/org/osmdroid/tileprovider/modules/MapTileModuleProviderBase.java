...
import com.nextgis.mobile.datasource.OpenStreetMapTileProviderConstants;
...
public abstract class MapTileModuleProviderBase implements OpenStreetMapTileProviderConstants {
	...
	public MapTileModuleProviderBase(int pThreadPoolSize, final int pPendingQueueSize) {
		if (pPendingQueueSize < pThreadPoolSize) {
			Log.w(OSMD_TAG, "The pending queue size is smaller than the thread pool size. Automatically reducing the thread pool size.");
	...
	public void loadMapTileAsync(final MapTileRequestState pState) {
		synchronized (mQueueLockObject) {
			if (DEBUG_TILE_PROVIDERS) {
				Log.d(OSMD_TAG, "MapTileModuleProviderBase.loadMaptileAsync() on provider: "
						+ getName() + " for tile: " + pState.getMapTile());
				if (mPending.containsKey(pState.getMapTile()))
					Log.d(OSMD_TAG, "MapTileModuleProviderBase.loadMaptileAsync() tile already exists in request queue for modular provider. Moving to front of queue.");
				else
					Log.d(OSMD_TAG, "MapTileModuleProviderBase.loadMaptileAsync() adding tile to request queue for modular provider.");
			}
		...
		try {
			...
			Log.w(OSMD_TAG, "RejectedExecutionException. " + e);
		}
	}
	...
	void removeTileFromQueues(final MapTile mapTile) {
		synchronized (mQueueLockObject) {
			if (DEBUG_TILE_PROVIDERS) {
				Log.d(OSMD_TAG, "MapTileModuleProviderBase.removeTileFromQueues() on provider: "
	...
	protected abstract class TileLoader implements Runnable {
		...
		protected MapTileRequestState nextTile() {
			...
			synchronized (mQueueLockObject) {
				...
				while (iterator.hasNext()) {
					...
					if (!mWorking.containsKey(tile)) {
						if (DEBUG_TILE_PROVIDERS) {
							Log.d(OSMD_TAG, "TileLoader.nextTile() on provider: " + getName()
		...
		protected void tileLoaded(final MapTileRequestState pState, final Drawable pDrawable) {
			if (DEBUG_TILE_PROVIDERS) {
				Log.d(OSMD_TAG, "TileLoader.tileLoaded() on provider: " + getName() + " with tile: "
						+ pState.getMapTile());
			}
		...
		protected void tileLoadedExpired(final MapTileRequestState pState, final Drawable pDrawable) {
			if (DEBUG_TILE_PROVIDERS) {
				Log.d(OSMD_TAG, "TileLoader.tileLoadedExpired() on provider: " + getName()
		...
		protected void tileLoadedFailed(final MapTileRequestState pState) {
			if (DEBUG_TILE_PROVIDERS) {
				Log.d(OSMD_TAG, "TileLoader.tileLoadedFailed() on provider: " + getName()
						+ " with tile: " + pState.getMapTile());
			}
			removeTileFromQueues(pState.getMapTile());
		...
		final public void run() {
			...
			while ((state = nextTile()) != null) {
				if (DEBUG_TILE_PROVIDERS) {
					Log.d(OSMD_TAG, "TileLoader.run() processing next tile: " + state.getMapTile());
				}
				try {
					result = null;
					result = loadTile(state);
				} catch (final CantContinueException e) {
					Log.i(OSMD_TAG, "Tile loader can't continue: " + state.getMapTile(), e);
					clearQueue();
				} catch (final Throwable e) {
					Log.e(OSMD_TAG, "Error downloading tile: " + state.getMapTile(), e);
				}
