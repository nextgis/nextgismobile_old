...
public class MapTileFileArchiveProvider extends MapTileFileStorageProviderBase {
	...
	private synchronized InputStream getInputStream(final MapTile pTile,
		...
		for (final IArchiveFile archiveFile : mArchiveFiles) {
			if (DEBUGMODE) {
				Log.d(OSMD_TAG, "Search tile " + pTile + " in " + archiveFile);
			}
			final InputStream in = archiveFile.getInputStream(tileSource, pTile);
			if (in != null) {
				if (DEBUGMODE) {
					Log.d(OSMD_TAG, "Found tile " + pTile + " in " + archiveFile);
				}
				return in;
			}
		}
	...
	protected class TileLoader extends MapTileModuleProviderBase.TileLoader {
		...
		public Drawable loadTile(final MapTileRequestState pState) {
			...
			if (!getSdCardAvailable()) {
				if (DEBUGMODE) {
					Log.d(OSMD_TAG, "No sdcard - do nothing for tile: " + pTile);
				}
				return null;
			}
			...
			try {
				if (DEBUGMODE) {
					Log.d(OSMD_TAG, "Tile doesn't exist: " + pTile);
				}

				inputStream = getInputStream(pTile, tileSource);
				if (inputStream != null) {
					if (DEBUGMODE) {
						Log.d(OSMD_TAG, "Use tile from archive: " + pTile);
					}
					final Drawable drawable = tileSource.getDrawable(inputStream);
					return drawable;
				}
			} catch (final Throwable e) {
				Log.e(OSMD_TAG, "Error loading tile. " + e.getLocalizedMessage());
			} finally {
				if (inputStream != null) {
