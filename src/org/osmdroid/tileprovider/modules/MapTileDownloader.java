	...
	protected class TileLoader extends MapTileModuleProviderBase.TileLoader {
		...
		public Drawable loadTile(final MapTileRequestState aState) throws CantContinueException {
			...
			try {
				...
				if (DEBUGMODE) {
					Log.d(OSMD_TAG, "Downloading Maptile from url: " + tileURLString);
				}
				...
				if (line.getStatusCode() != 200) {
					Log.w(OSMD_TAG, "Problem downloading MapTile: " + tile + " HTTP response: " + line);
					return null;
				}

				final HttpEntity entity = response.getEntity();
				if (entity == null) {
					Log.w(OSMD_TAG, "No content downloading MapTile: " + tile);
					return null;
				}
				...
				Log.w(OSMD_TAG, "UnknownHostException downloading MapTile: " + tile + " : " + e);
				throw new CantContinueException(e);
			} catch (final LowMemoryException e) {
				// low memory so empty the queue
				Log.w(OSMD_TAG, "LowMemoryException downloading MapTile: " + tile + " : " + e);
				throw new CantContinueException(e);
			} catch (final FileNotFoundException e) {
				Log.w(OSMD_TAG, "Tile not found: " + tile + " : " + e);
			} catch (final IOException e) {
				Log.w(OSMD_TAG, "IOException downloading MapTile: " + tile + " : " + e);
			} catch (final Throwable e) {
				Log.e(OSMD_TAG, "Error downloading MapTile: " + tile, e);
			} finally {
